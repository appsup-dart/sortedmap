...
class TreeMap<K extends Comparable,V> extends MapBase<K,V> {
  ...
  factory TreeMap.from(Map<K,V> other) => new TreeMap()..addAll(other);

  static int _compareKeys<K extends Comparable,V>(Pair<K,V> a, Pair<K,V> b) => Comparable.compare(a.key, b.key);

  final TreeSet<Pair<K,V>> _tree = new TreeSet(comparator: _compareKeys);
  ...
  void addAll(Map<K,V> other) {
    if (other is TreeMap<K,V>) _tree.addAll(other._tree);
    else {
      super.addAll(other);
    }
  }
