...
abstract class FilteredMap<K extends Comparable,V> implements SortedMap<K,V> {
  ...
  KeyValueInterval<K,Comparable> get completeInterval {
    ...
    if (filter.limit==null||keys.length<filter.limit) return filterInterval;
    if (filter.limit==0) return null;

    if (filter.reversed) {
      return new KeyValueInterval.fromPairs(_pairForKey(keys.first), filterInterval.end);
    } else {
      return new KeyValueInterval.fromPairs(filterInterval.start, _pairForKey(keys.last));
    }
  }
...
class _FilteredMap<K extends Comparable,V> extends _SortedMap<K,V> with FilteredMap<K,V> {

  @override
  final Filter<K,V> filter;

  _FilteredMap._(Filter<K,V> filter, TreeSet<Pair<K,Comparable>> sortedPairs, Map<K,V> map) :
        filter = filter, super._(filter.ordering, sortedPairs, map);


  @override
  FilteredMap<K,V> clone() => new _FilteredMap<K,V>._(this.filter, new TreeSet()..addAll(_sortedPairs), new Map.from(_map));


  @override
  void _addPair(K key, V value) {
    if (!filter.validInterval.containsPoint(ordering.mapKeyValue(key,value))) return;
    super._addPair(key,value);
    if (filter.limit!=null&&length>filter.limit) {
      var toDel = filter.reversed ? _sortedPairs.take(length-filter.limit)
...
class FilteredMapView<K extends Comparable,V> extends MapBase<K,V> with SortedMap<K,V>, FilteredMap<K,V>, UnmodifiableSortedMap<K,V> {
  ...
  FilteredMapView(this._baseMap, {Pair<K,Comparable> start, Pair<K,Comparable> end, int limit, bool reversed}) :
    filter = new Filter(validInterval: new KeyValueInterval.fromPairs(start, end), limit: limit, reversed: reversed, ordering: _baseMap.ordering);
  ...
  bool _containsPair(Pair<K,Comparable> pair) => _effectiveInterval.containsPoint(pair);

  KeyValueInterval<K,Comparable> get _effectiveInterval {
    var keys = this.keys;
    return new KeyValueInterval.fromPairs(_pairForKey(keys.first, false),_pairForKey(keys.last,false));
  }
  ...
  @override
  Iterable<K> get keys => _baseMap.subkeys(start: filter.validInterval.start,
  ...
  bool get isComplete {
    ...
    return (_baseMap as FilteredMap).completeInterval.contains(completeInterval);
  }
